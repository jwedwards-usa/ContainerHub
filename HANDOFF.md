...
The all-worker reconciliation preserves the manufacturer/industrial waves, retailer breadth waves, five recovered Really Useful Box sizes, the Uline shelf-bin recovery, the full standard Cambro FreshPro CamRound family, and the late Akro-Grid worker recovery.
...
The FreshPro CamRound wave covers all seven current standard translucent polypropylene sizes from 2–22 qt with Cambro exterior dimensions and model-matched WebstaurantStore purchase links. The recovered Akro-Grid wave adds 12 current dividable storage boxes across two shards, retaining published external/interior dimensions, capacity and load ratings while leaving unsupported tare weights null.
